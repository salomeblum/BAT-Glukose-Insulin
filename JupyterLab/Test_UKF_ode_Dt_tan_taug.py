...
def f_odes_Dt_tan_taug(x: np.ndarray, t: float, D_t : np.ndarray) -> np.ndarray:
    """
    ...
    #Reparametrisierung tau_g mit Tangens
    p_taug = x[4] #von UKF geschätzer Parameter
    a_taug, b_taug = (1e-10)*60, 20*60 #lower und upper bound für Parameter tau_g
    tau_g_t = (a_taug + b_taug)/2 + (b_taug - a_taug)/np.pi*np.arctan(p_taug)

    #In der Simulation verwendete Werte für k1 und k5 
    k1_t = 0.0002475
    k5_t = 7.599999999999999e-05
    ...
    return np.array([dM_gut, dG_pl, dI_pl, dG_i, 0.0])
