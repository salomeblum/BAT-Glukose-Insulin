...
import personal_params as param
...
def f_odes_Dt_sim(x: np.ndarray, t: float, D_t : np.ndarray) -> np.ndarray:
    """
    ...
    #print(D_t) #Debug-Ausgabe: Überprüfung des Inhalts von D_t
    ...
    for ti, Di in D_t: #Aufsummieren Mahlzeiteninput zum Zeitpunkt "time"
        #print(f"f_odes_Dt_sim: t: {t}") #Debug-Ausgabe: Überprüfung ob t korrekt inkrementiert
        if t > ti:
            delta_t = t - ti
            #print(f"Delta_t = {delta_t}") #Debug-Ausgabe: Überprüfung, ob die Bedingung je erfüllt wird.
