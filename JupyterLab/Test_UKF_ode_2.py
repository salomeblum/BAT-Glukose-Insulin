...
#Mahlzeitinput zum Zeitpunkt t=0
#Erlaubt die Veränderung der Parameter k1, k5 und tau_g durch Hinzufügen eines Versatzes zum Literaturwert
#Ausgabe dM_gut, dG_pl, dI_pl, dG_i, delta k1, delta k5, delta tau_g

def f_odes_2(x: np.ndarray, t: float, D: int) -> np.ndarray:
    """
    ...
    k1 = param.k1+x[4]
    k5 = param.k5+x[5]
    tau_g = param.tau_g+x[6]
    

    #Flüsse Magen
    m_meal = param.sigma * (k1**param.sigma) * (t**(param.sigma-1))*np.exp(-((k1*t)**param.sigma))*D #Glukose-Aufnahme Magen
    ...
    c2 = param.g_liv_b*((param.K_M+param.G_pl_b)/param.G_pl_b)-param.I_pl_b*k5*param.beta #Hilfskonstante zur Berechnung von g_non_it
    ...
    g_it = k5*param.beta*I_pl*G_pl/(param.K_M+G_pl)
...
if __name__ == "__main__":
    ...
    k5 = param.k5
    tau_g = param.tau_g
    x = np.array([0,G_start,I_start,G_start, k1, k5, tau_g])
    
    for t in np.arange(1, 86400, dt):
        dxdt = f_odes_2(x, t, 75000)
        print(dxdt)
        """
