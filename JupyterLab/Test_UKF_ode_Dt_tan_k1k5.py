...
def f_odes_Dt_tan_k1k5(x: np.ndarray, t: float, D_t : np.ndarray) -> np.ndarray:
    """
    ...
    p_k1 = x[4] #von UKF geschätzer Parameterinput
    a_k1, b_k1 = 1e-7/60, 3e-1/60 #lower und upper bound für Parameter k1
    k1_t = (a_k1 + b_k1)/2 + (b_k1 - a_k1)/np.pi*np.arctan(p_k1)
    
    #Reparametrisierung k5 mit Tangens
    p_k5 = x[5] #von UKF geschätzer Parameterinput
    a_k5, b_k5 = 1e-7/60, 1.0/60 #lower und upper bound für Parameter k5
    k5_t = (a_k5 + b_k5)/2 + (b_k5 - a_k5)/np.pi*np.arctan(p_k5)

    #In der Simulation verwendeter Wert für tau_g
    tau_g_t = 180
    ...
    return np.array([dM_gut, dG_pl, dI_pl, dG_i, 0.0, 0.0])
